...
class AlpineApkFile(File):
    ...
    CONTAINER_CLASSES = []  # Will be dynamically set based on APK version
    ...
    def as_container(self):
        ...
        if version == 3:
            self._as_container = AlpineApkV3Container(self)
        elif version == 2:
            self._as_container = AlpineApkV2Container(self)
        else:
    ...
    def cleanup(self):
        """Remove any temporary data associated with the file."""
        if hasattr(self, "_as_container"):
            if hasattr(self._as_container, 'close_archive'):
                try:
                    self._as_container.close_archive()
                except Exception:
                    pass  # Ignore cleanup errors
            delattr(self, "_as_container")
        super().cleanup()
